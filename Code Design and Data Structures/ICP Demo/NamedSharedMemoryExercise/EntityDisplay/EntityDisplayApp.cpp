...
bool EntityDisplayApp::startup() {
	...
	HANDLE numEntitiesHandle = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, L"EntityCountMemory");
	assert(numEntitiesHandle != nullptr);

	m_numEntityPtr = (int*)MapViewOfFile(numEntitiesHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(int));

	m_entities.resize(*m_numEntityPtr);

	m_EntityFileHandle = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, L"EntityFileMemory");
	assert(m_EntityFileHandle != nullptr);
