...
bool EntityEditorApp::startup() {
	...
	m_numEntitiesHandle = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(int), L"EntityCountMemory");
	//Create a pointer to it
	m_numEntityPtr = (int*)MapViewOfFile(m_numEntitiesHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(int));
	//Use the pointer to set the number of Entities file to equal the entity counht
	*m_numEntityPtr = ENTITY_COUNT;

	//Open the file that holds the entity data
	m_entityFileHandle = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr,  PAGE_READWRITE, 0, sizeof(Entity) * ENTITY_COUNT, L"EntityFileMemory");
	//Create a pointer to it
	m_EntityFilePtr = (Entity*)MapViewOfFile(m_entityFileHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(Entity)* ENTITY_COUNT);
	

	return true;
}

void EntityEditorApp::shutdown() { //Unmaps files, deletes pointers cleans up
...
void EntityEditorApp::update(float deltaTime) {
	...
	//Copies entity data from the entity file 
	memcpy(m_EntityFilePtr, m_entities, sizeof(Entity)* ENTITY_COUNT);
